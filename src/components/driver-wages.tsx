...
function ExtraMaterialsManager({
  materials,
  loading,
  onRefresh,
}: {
  ...
  async function handleSave(
    id: string | null,
    values: { name: string; keywords: string[]; price_tonne: number; price_unit: number },
  ) {
    setSavingId(id ?? "new");
    if (id) {
                ...
                <div key={m.id} className="flex items-center justify-between rounded-lg border border-border p-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      طن: {m.price_tonne.toLocaleString()} · قطعة: {m.price_unit.toLocaleString()}
                    </div>
                    <div className="text-xs text-muted-foreground opacity-60">{m.keywords.join(" · ")}</div>
                  </div>
                  <div className="flex gap-2">
                    <Button size="icon" variant="outline" onClick={() => setEditingId(m.id)}>
...
function DriverForm({
  ...
  initial?: Driver;
  onCancel: () => void;
  onSave: (v: DriverFormValues) => void;
  saving: boolean;
}) {
  const [name, setName] = useState(initial?.name ?? "");
...
export function DriverWages() {
  ...
  async function loadDrivers() {
    ...
    if (error) {
      toast.error("فشل تحميل السائقين: " + error.message);
      return;
    }
  ...
  async function loadExtraMaterials() {
    ...
    if (error) {
      toast.error("فشل تحميل المواد الإضافية: " + error.message);
      return;
    }
  ...
  async function calculate() {
    ...
    try {
      ...
      const { data: orders, error } = await supabase
        .from("orders")
        .select("id, details, driver_name, delivered_at")
      ...
      for (const driver of drivers) {
        ...
        const deliveryTotal =
          gemini.ciment_tonnes * deliveryRates.ciment_tonne +
          gemini.barigs * deliveryRates.barig +
          gemini.fer_tonnes * deliveryRates.fer_tonne;
        ...
        driverResults.push({
          name: driver.name,
          ciment_tonnes: gemini.ciment_tonnes,
          barigs: gemini.barigs,
          fer_tonnes: gemini.fer_tonnes,
          deliveryRates,
          deliveryTotal,
          orders: driverOrders,
          recCiment,
                ...
                {r.orders.length > 0 && (
                  <details className="text-xs text-muted-foreground">
                    ...
                    <ul className="mt-2 space-y-1 ps-4">
                      {r.orders.map((o, i) => (
                        <li key={o.id} style={{ direction: "ltr", textAlign: "left" }}>
                          {i + 1}. {o.details?.slice(0, 100)}
                        </li>
                      ))}
                    </ul>
                  </details>
                  ...
                  <summary className="cursor-pointer p-3 text-sm font-bold list-none flex justify-between items-center">
                    ...
                    {r.receptionTotal > 0 && (
                      <span dir="ltr" className="text-muted-foreground font-normal">
                        {r.receptionTotal.toLocaleString()} MRO
                      </span>
                    )}
                  </summary>
                    ...
                    {r.receptions.length > 0 && (
                      <ul className="mt-2 space-y-1 ps-4 text-xs text-muted-foreground">
                        {r.receptions.map((rec, i) => (
                          <li key={rec.id} style={{ direction: "ltr", textAlign: "left" }}>
